...
!!! info "Instructions"
    ...
    This exam follows the official GH-600 domain weightings and question styles. Includes practical CLI and configuration questions.
...
<div class="quiz-option" data-correct="false">Allow the agent to plan and execute in a single step for efficiency with token budget allocated from the organization pool</div>
...
<div class="quiz-option" data-correct="false">Hallucination — switch to a more capable model, which typically improves response accuracy at higher latency cost</div>
<div class="quiz-option" data-correct="false">Context overflow — reduce the agent's memory window, with automatic eviction of entries beyond the configured retention</div>
...
<div class="quiz-option" data-correct="true">Classify actions by risk level</div>
...
<div class="quiz-option" data-correct="false">The agent must generate PDF reports after each task, checked against the configured boundaries using a task-specific identity token</div>
<div class="quiz-option" data-correct="true">The agent records its plans, decisions, and actions as pull request comments, commit messages, workflow logs, or issue updates</div>
<div class="quiz-option" data-correct="false">The agent must create a separate dashboard application and snapshots captured between each step</div>
...
<div class="quiz-option" data-correct="false">Post-execution auditing, subject to the model routing rules set by the admin</div>
<div class="quiz-option" data-correct="true">Pre-execution plan validation with defined constraints</div>
<div class="quiz-option" data-correct="false">Runtime sandboxing with progress saved after each operation</div>
...
<div class="quiz-option" data-correct="false">Having the agent create branches for its work with temperature and top-p tuned for the task type</div>
<div class="quiz-option" data-correct="false">Requiring code review on agent-generated PRs, subject to the model routing rules set by the admin</div>
...
<div class="quiz-option" data-correct="false">Give full autonomy on development branches and no autonomy on main, inheriting the parent agent configuration by default</div>
<div class="quiz-option" data-correct="false">Autonomy level should be fixed and never change with temperature and top-p tuned for the task type</div>
...
<div class="quiz-option" data-correct="false">Only allow the agent to read issues, not modify them at all, inheriting the parent agent configuration by default</div>
<div class="quiz-option" data-correct="false">Let the agent decide its own scope based on the issue content with token budget allocated from the organization pool and validated against the agent capability manifest</div>
...
<div class="quiz-option" data-correct="false">To slow down the agent and reduce compute costs, validated by the policy enforcement layer, with execution traces recorded for post-hoc analysis</div>
<div class="quiz-option" data-correct="false">To generate documentation automatically with token budget allocated from the organization pool</div>
...
<div class="quiz-option" data-correct="false">Add allowed servers to the repository's README, limited to the current repo scope by default</div>
<div class="quiz-option" data-correct="true">Define the allowed MCP servers in the agent's configuration, specifying which servers the agent is permitted to connect to</div>
...
<div class="quiz-explanation">MCP allow lists restrict which servers an agent can connect to. Only explicitly listed servers are permitted, for external tool access.</div>
</div>
</div>
...
<div class="quiz-option" data-correct="false">Remote servers are faster than local servers, which addresses a common misconception about this feature</div>
<div class="quiz-option" data-correct="true">Remote MCP servers run as hosted services accessible over HTTP/SSE</div>
<div class="quiz-option" data-correct="false">Local servers support more tools than remote servers, as described in the original design specification</div>
<div class="quiz-option" data-correct="false">Remote servers don't require authentication using lazy initialization for resource efficiency</div>
...
<div class="quiz-option" data-correct="false">Add a comment in the agent's prompt asking it to stay in src/, as described in the original design specification</div>
<div class="quiz-option" data-correct="true">Configure the agent's scope to the specific repository with branch-based filtering (exclude main)</div>
<div class="quiz-option" data-correct="false">Only run the agent on a fork of the repository, simplifying the architecture at the cost of flexibility</div>
<div class="quiz-option" data-correct="false">Use a post-commit hook to revert changes outside src/, resolved from the tool registry at invocation time</div>
...
<div class="quiz-option" data-correct="false">Immediately alert the user on any failure with temperature and top-p tuned for the task type</div>
...
<div class="quiz-option" data-correct="false">Retry indefinitely until the operation succeeds, inheriting the parent agent configuration by default</div>
...
<div class="quiz-option" data-correct="false">Recording only whether the agent succeeded or failed, logging each attempt for debugging and pattern detection</div>
<div class="quiz-option" data-correct="true">Recording each tool call with its parameters, results, timestamps, the decision reasoning, and the identity of who triggered the agent</div>
<div class="quiz-option" data-correct="false">Storing the complete model weights used for each decision, allowing the system to handle more complex reasoning chains</div>
<div class="quiz-option" data-correct="false">Only logging actions that resulted in errors per the agent delegation and escalation policy</div>
...
<div class="quiz-option" data-correct="true">Configure environment-aware permissions</div>
...
<div class="quiz-option" data-correct="true">Declare the MCP server in the agent's configuration file (e.g.</div>
<div class="quiz-option" data-correct="false">Ask Copilot in chat to connect to the MCP server, which addresses a common misconception about this feature</div>
<div class="quiz-option" data-correct="false">MCP servers are built into Copilot and don't need configuration, defaulting to the repository boundary</div>
...
<div class="quiz-option" data-correct="false">Retry the push with elevated permissions and state preserved at decision points</div>
<div class="quiz-option" data-correct="false">Disable branch protection temporarily, compared to the defined guardrail criteria authenticated as the task-bound principal</div>
<div class="quiz-option" data-correct="true">Create a new feature branch, push the changes there, and open a pull request targeting the protected branch</div>
...
<div class="quiz-option" data-correct="true">Only enable the specific tools the agent needs for its defined task</div>
...
<div class="quiz-option" data-correct="false">To store agent conversation history with connection pooling enabled for concurrent requests</div>
<div class="quiz-option" data-correct="true">To provide a catalog of available MCP servers that agents can discover and connect to</div>
<div class="quiz-option" data-correct="false">To register domain names for MCP endpoints, with request signing using the configured service credentials</div>
<div class="quiz-option" data-correct="false">To track billing for MCP server usage, aggregated in a dashboard for real-time visibility</div>
...
<div class="quiz-option" data-correct="false">Retry immediately 100 times until it succeeds, subject to the model routing rules set by the admin</div>
<div class="quiz-option" data-correct="false">Skip the dependency and continue with the rest of the update</div>
<div class="quiz-option" data-correct="false">Switch to a different package manager automatically with temperature and top-p tuned for the task type and surfaced in the observability dashboard</div>
...
<div class="quiz-option" data-correct="false">Delete the entire repository and restore from backup, inheriting the parent agent configuration by default, with execution traces recorded for post-hoc analysis</div>
...
<div class="quiz-option" data-correct="false">Rollback is not possible for agent operations with temperature and top-p tuned for the task type</div>
...
<div class="quiz-option" data-correct="false">The execution context is the LLM model version being used, inheriting the parent agent configuration by default</div>
<div class="quiz-option" data-correct="true">The execution context includes the repository state, branch, available tools, permissions, environment variables, and runtime constraints</div>
<div class="quiz-option" data-correct="false">The execution context is the user's IDE theme and settings with token budget allocated from the organization pool using the standard agent handoff protocol for state transfer</div>
...
<div class="quiz-option" data-correct="false">They differ only in storage capacity with token budget allocated from the organization pool, with parallel execution across the configured test matrix</div>
<div class="quiz-option" data-correct="true">Short-term memory is the current conversation/task context window; long-term memory persists across sessions (e.g.</div>
<div class="quiz-option" data-correct="false">Short-term is RAM, long-term is disk, external is cloud, tested against the safety threshold, limited by the defined permission scope</div>
<div class="quiz-option" data-correct="false">All three types are identical but stored in different locations, which addresses a common misconception about this feature</div>
...
<div class="quiz-option" data-correct="false">The model is producing random outputs — restart it with intermediate results cached locally</div>
<div class="quiz-option" data-correct="true">Context drift — the agent's earlier decisions have fallen out of its context window</div>
<div class="quiz-option" data-correct="false">The agent is testing different approaches — let it continue</div>
<div class="quiz-option" data-correct="false">Increase the model's temperature setting, measured against the compliance rules</div>
...
<div class="quiz-option" data-correct="false">Include the entire repository history in the agent's context per the agent delegation and escalation policy</div>
<div class="quiz-option" data-correct="true">Filter memory to include only information relevant to the current task</div>
...
<div class="quiz-option" data-correct="true">When the task is complete (prune task-specific details)</div>
<div class="quiz-option" data-correct="false">Never — all memory should be preserved indefinitely, compressing older entries to maintain the most relevant signals</div>
<div class="quiz-option" data-correct="false">After every single agent action using composite actions for reusability</div>
<div class="quiz-option" data-correct="false">Only when the user manually requests it, subject to the model routing rules set by the admin</div>
...
<div class="quiz-option" data-correct="false">Replay the entire conversation from the beginning, which addresses a common misconception about this feature</div>
<div class="quiz-option" data-correct="true">Capture task progress and decisions as durable artifacts (e.g.</div>
<div class="quiz-option" data-correct="false">Store the complete model state and restore it, subject to the model routing rules set by the admin with the associated metadata preserved for traceability</div>
<div class="quiz-option" data-correct="false">Ask the user to summarize what happened before with temperature and top-p tuned for the task type, as described in the original design specification</div>
...
<div class="quiz-option" data-correct="false">Give each agent its own copy of all state and never synchronize, inheriting the parent agent configuration by default</div>
...
<div class="quiz-option" data-correct="false">Only allow one agent to run at a time across the entire organization, simplifying the architecture at the cost of flexibility</div>
<div class="quiz-option" data-correct="false">Conflicting context is acceptable and doesn't need prevention with temperature and top-p tuned for the task type, with automatic eviction of entries beyond the configured retention</div>
...
<div class="quiz-option" data-correct="false">Stale context is caused by slow network connections, inheriting the parent agent configuration by default</div>
<div class="quiz-option" data-correct="true">Stale context occurs when the agent's memory references information that has changed (files modified by others, merged PRs, updated configs)</div>
<div class="quiz-option" data-correct="false">Stale context only happens with long-term memory, never short-term</div>
<div class="quiz-option" data-correct="false">Stale context is prevented by using a larger context window with token budget allocated from the organization pool and validated against the agent capability manifest</div>
...
<div class="quiz-option" data-correct="false">Embedding state in the model's system prompt permanently, with execution traces recorded for post-hoc analysis</div>
...
<div class="quiz-option" data-correct="true">Quantitative signals are measurable metrics (test pass rate</div>
<div class="quiz-option" data-correct="false">Quantitative signals only apply to performance, qualitative only to security, simplifying the architecture at the cost of flexibility</div>
<div class="quiz-option" data-correct="false">They are the same thing measured differently with token budget allocated from the organization pool, as described in the original design specification</div>
...
<div class="quiz-option" data-correct="true">The evaluation criteria are misaligned with development intent</div>
...
<div class="quiz-option" data-correct="true">Linters (style compliance), SAST/DAST (security)</div>
<div class="quiz-option" data-correct="false">Only the same LLM that generated the code can evaluate it with alerts routed to the security team channel, operating within the declared task boundary</div>
<div class="quiz-option" data-correct="false">Scanning tools cannot be used to evaluate AI-generated code, which addresses a common misconception about this feature</div>
...
<div class="quiz-option" data-correct="false">Model laziness — the agent intentionally chose not to work and rollback points created automatically per the agent delegation and escalation policy</div>
...
<div class="quiz-option" data-correct="true">Environment issue (missing permissions)</div>
...
<div class="quiz-option" data-correct="true">Examine logs (step-by-step execution), plans (intended vs</div>
...
<div class="quiz-option" data-correct="false">Remove all database tools from the agent with temperature and top-p tuned for the task type</div>
<div class="quiz-option" data-correct="true">Refine tool access — clarify tool descriptions so the agent can distinguish between them</div>
<div class="quiz-option" data-correct="false">Increase the agent's context window size, subject to the model routing rules set by the admin</div>
<div class="quiz-option" data-correct="false">Add more tools to give the agent more options, which addresses a common misconception about this feature</div>
...
<div class="quiz-option" data-correct="true">Update the agent's system instructions to be clearer, modify workflow steps to add validation checkpoints, or adjust constraints to be more specific</div>
<div class="quiz-option" data-correct="false">Completely rewrite the agent from scratch after each failure, with configurable backoff intervals between successive attempts</div>
<div class="quiz-option" data-correct="false">Add more constraints until the agent can barely do anything, which addresses a common misconception about this feature</div>
<div class="quiz-option" data-correct="false">Remove all instructions and let the agent figure things out with credentials isolated to this execution</div>
...
<div class="quiz-option" data-correct="true">The evaluation set is not representative of real usage</div>
...
<div class="quiz-option" data-correct="false">Always maximize memory — more context is always better, compressing older entries to maintain the most relevant signals</div>
<div class="quiz-option" data-correct="true">Analyze whether failures correlate with missing context (add relevant memory) or context overload (prune irrelevant memory)</div>
<div class="quiz-option" data-correct="false">Set memory to a fixed size regardless of task type with token budget allocated from the organization pool</div>
<div class="quiz-option" data-correct="false">Disable all memory between tasks, inheriting the parent agent configuration by default, with an admin override available for emergency situations only</div>
...
<div class="quiz-option" data-correct="false">Agents cannot communicate with each other at all with token budget allocated from the organization pool and validated against the agent capability manifest</div>
<div class="quiz-option" data-correct="true">Each parallel agent operates in its own execution context (separate branches, sandboxed workspaces, or isolated file sets) to prevent interference</div>
<div class="quiz-option" data-correct="false">Only one agent can run at a time across the system</div>
<div class="quiz-option" data-correct="false">Agents are isolated on different physical machines, validated by the policy enforcement layer</div>
...
<div class="quiz-option" data-correct="false">Always accept the most recent change, compared to the defined guardrail criteria, weighted by the domain-specific importance factors</div>
<div class="quiz-option" data-correct="true">Detect overlapping changes during merge (file-level or function-level conflict detection)</div>
<div class="quiz-option" data-correct="false">Delete both agents' work and start over and snapshots captured between each step, generating a summary report at each transition point</div>
<div class="quiz-option" data-correct="false">Merge conflicts are impossible with proper agent configuration, validated against the expected schema before application</div>
...
<div class="quiz-option" data-correct="false">Only the final merged output using composite actions for reusability, reducing coordination overhead but limiting scalability</div>
<div class="quiz-option" data-correct="true">Per-agent decision logs with correlation IDs linking related actions across agents</div>
<div class="quiz-option" data-correct="false">A single combined log file with all agent outputs mixed together, creating a backup of the original state before each modification</div>
<div class="quiz-option" data-correct="false">Observability is only needed for single-agent workflows using the standard agent handoff protocol for state transfer</div>
...
<div class="quiz-option" data-correct="false">Let Agent C complete the deployment anyway, subject to the model routing rules set by the admin, restricted to the approved operation set</div>
<div class="quiz-option" data-correct="true">Implement a rollback pattern: halt Agent C immediately</div>
<div class="quiz-option" data-correct="false">Restart all three agents from the beginning with temperature and top-p tuned for the task type</div>
<div class="quiz-option" data-correct="false">Only notify the team without taking corrective action, simplifying the architecture at the cost of flexibility</div>
...
<div class="quiz-option" data-correct="true">Deploy the new agent alongside existing ones in a shadow/observation mode first</div>
<div class="quiz-option" data-correct="false">Stop all existing workflows, add the new agent, and restart everything, as described in the original design specification</div>
<div class="quiz-option" data-correct="false">Immediately add the agent to the production workflow per the agent delegation and escalation policy</div>
<div class="quiz-option" data-correct="false">New agents cannot be added to existing workflows, which addresses a common misconception about this feature</div>
...
<div class="quiz-option" data-correct="false">Nothing — just remove it and let other agents adapt, as described in the original design specification</div>
<div class="quiz-option" data-correct="true">Auditability (logs and artifacts from past executions remain accessible)</div>
<div class="quiz-option" data-correct="false">Only the agent's source code needs to be preserved, simplifying the architecture at the cost of flexibility</div>
...
<div class="quiz-option" data-correct="false">Wait until the user complains about missing results, as described in the original design specification</div>
<div class="quiz-option" data-correct="true">Monitor execution time against expected duration thresholds</div>
<div class="quiz-option" data-correct="false">Stalled agents will always throw an error, logging each attempt for debugging and pattern detection</div>
<div class="quiz-option" data-correct="false">Check if the agent's CPU usage is zero running under the task-scoped identity</div>
...
<div class="quiz-option" data-correct="false">A single agent processes tasks sequentially with progress saved after each operation</div>
<div class="quiz-option" data-correct="true">Work is split into independent subtasks (fan-out) executed in parallel by multiple agents, then results are collected and combined (fan-in)</div>
<div class="quiz-option" data-correct="false">Agents vote on the correct output, tested against the safety threshold, with parallel execution across the configured test matrix</div>
<div class="quiz-option" data-correct="false">Each agent's output becomes the next agent's input sequentially, which addresses a common misconception about this feature</div>
...
<div class="quiz-option" data-correct="false">Update CodeGen immediately — other agents should handle format changes automatically, which addresses a common misconception about this feature</div>
<div class="quiz-option" data-correct="true">Update CodeGen with the new standard</div>
<div class="quiz-option" data-correct="false">Rebuild all three agents from scratch with the new standard</div>
<div class="quiz-option" data-correct="false">Keep using the old standard indefinitely to avoid risk and state preserved at decision points, with parallel execution across the configured test matrix</div>
...
<div class="quiz-option" data-correct="false">By the time they take to execute, cached between workflow runs for faster startup</div>
<div class="quiz-option" data-correct="true">By operational risk (impact scope), security risk (data sensitivity, system access), and compliance risk (regulatory requirements)</div>
<div class="quiz-option" data-correct="false">By the model's confidence score for each action, with execution traces recorded for post-hoc analysis</div>
...
<div class="quiz-option" data-correct="false">To minimize all agent actions for maximum safety using the standard agent handoff protocol for state transfer</div>
<div class="quiz-option" data-correct="true">To maximize delivery speed while remaining compliant with organizational security and Responsible AI standards</div>
<div class="quiz-option" data-correct="false">To maximize agent autonomy regardless of risk with temperature and top-p tuned for the task type</div>
...
<div class="quiz-option" data-correct="false">Running unit tests in a development environment, correlated with the task complexity classification</div>
...
<div class="quiz-option" data-correct="false">A performance guardrail with the workflow_dispatch trigger for manual override per the documented operational runbook</div>
<div class="quiz-option" data-correct="true">A security and compliance guardrail that blocks actions violating defined security policies</div>
<div class="quiz-option" data-correct="false">A style guardrail for code quality</div>
<div class="quiz-option" data-correct="false">An authentication guardrail, pinned to a specific runner image version</div>
...
<div class="quiz-option" data-correct="true">Grant only the specific permissions needed for the defined task scope</div>
...
<div class="quiz-option" data-correct="false">Never require approvals for anything, enforced through the branch ruleset configuration</div>
<div class="quiz-option" data-correct="true">Only require human approval for actions where the approval actually prevents meaningful harm</div>
<div class="quiz-option" data-correct="false">Require approval only from senior engineers and gated by the CODEOWNERS approval requirement</div>
...
<div class="quiz-option" data-correct="true">Require explicit authorization gates for these specific action categories</div>
...
<div class="quiz-option" data-correct="true">Guardrails are the implementation mechanism for Responsible AI policies</div>
...
<div class="quiz-option" data-correct="false">A webhook endpoint that receives issue events and calls the Copilot API authenticated with a session-scoped token</div>
